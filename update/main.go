...
func main() {
	...
	const aboutTitle = `<h2>💫 About me</h2>`
	const aboutText = `<p>I'm a Backend Engineer working in cybersecurity, with over 5 years of experience in the fintech industry. I have a strong interest in automation and improving process efficiencys. Right now I use Go, Python, JavaScript, GraphQL, SQL and Cassandra in my day-to-day work and side projects.</p>`
	...
	const githubStatsText = `![](https://github-readme-stats.vercel.app/api?username=Skyth3r&theme=city_light&hide_border=false&include_all_commits=false&count_private=true)<br/>![](https://github-readme-streak-stats.herokuapp.com/?user=Skyth3r&theme=city_light&hide_border=false)<br/>![](https://github-readme-stats.vercel.app/api/top-langs/?username=Skyth3r&theme=city_light&hide_border=false&include_all_commits=false&count_private=true&layout=compact)`
	...
	article := `<p>Latest article from my website: <a href="` + webFeedItem.Link + `">` + webFeedItem.Title + `</a>. Subscribe to <a href="` + webFeed + `">article RSS feed.</a></p>`
	devArticle := `<p>Latest post from my dev blog: <a href="` + devFeedItem.Link + `">` + devFeedItem.Title + `</a>. Subscribe to <a href="` + devFeed + `">dev post RSS feed.</a></p>`

	date := time.Now().Format("2 Jan 2006")
	updated := `<sub>Last updated on ` + date + `.<sub>`

	data := fmt.Sprintf("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n%s\n%s\n\n%s\n%s\n\n%s\n\n%s", title, aboutTitle, aboutText, socialLinks, latestPostsTitle, article, devArticle, techStackTitle, techStackText, githubStatsTitle, githubStatsText, githubContributionsTitle, githubContributionsText, updated)
